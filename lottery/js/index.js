	...
	var Lottery = function (_mix) {
	    ...
	    _createClass(Lottery, [{
	        ...
	        value: function updateState() {
	            var self = this;
	            self.issue = stateData.issue;
	            self.end_time = stateData.end_time;
	            self.state = stateData.state;
	            (0, _jquery2.default)(self.issue_el).text(stateData.issue);
	            self.countdown(stateData.end_time, function (time) {
	        ...
	        value: function initEvent() {
	            ...
	            console.log("test1:", openCodeData);
	            console.log("test2:", omitData);
	            console.log("test3:", stateData);
	        }
	    }]);

	    return Lottery;
	}(mix(_base2.default, _calculate2.default, _timer2.default));
	//因为要放在静态服务器上所以没有跑后台环境


	var mockjs = __webpack_require__(304);
	// get opencode
	var openCodeData = mockjs.mock({
	    'data': [/[1-3]/, /[4-5]/, /[6-7]/, /[8-9]/, /1[0-1]/]
	}).data;
	// get omit
	var omitData = mockjs.mock({
	    'data|11': [/[1-9]{1,3}|0/],
	    'issue': /[1-9]{8}/
	});
	// 获取状态
	var stateData = makeIssue2();
	// 生成获奖期数
	function makeIssue2() {
	...
	(function (global, factory) {
	  ...
	  */
		...
		var getStyles = function getStyles(elem) {
			...
			// FF meanwhile throws on eventLib elements through "defaultView.getComputedStyle"
		...
		function Animation(elem, properties, options) {
			var result,
				...
				stop: function stop(gotoEnd) {
					...
					// Resolve when we played the last eventLib; otherwise, reject
					if (gotoEnd) {
						deferred.notifyWith(elem, [animation, 1, 0]);
...
/***/ (function(module, exports) {
	...
	var Timer = function () {
	    ...
	    _createClass(Timer, [{
	        key: 'countdown',
	        value: function countdown(end, update, handler) {
